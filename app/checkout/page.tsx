...
function CheckoutContent() {
    ...
    // --- LOGIC SCAN: SEKARANG BISA SCAN STOK 0 ---
    async function onScanSuccess(decodedText: string) {
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                // JIKA STOK 0 ATAU MINUS, KASIH INFO TAPI TETAP MASUKIN LIST
                if (Number(item.stock_qty) <= 0) {
                    alert(`⚠️ PERHATIAN: Stok ${item.item_name} HABIS atau MINUS (${item.stock_qty}). \n\nBarang tetap masuk list, tapi Anda tidak bisa Submit Resmi sampai stok tersedia.`);
                }
                ...
                alert(result.message); // Menampilkan "Barang tidak terdaftar" jika QR salah
            }
    ...
    const handleSubmit = async (isDraft: boolean = false) => {
        ...
        // VALIDASI SUBMIT RESMI (TIDAK BOLEH ADA YANG MELEBIHI STOK)
        if (!isDraft) {
            for (const item of cart) {
                if (Number(item.qty) > Number(item.stock)) {
                    alert(`❌ GAGAL SUBMIT RESMI!\n\nBarang: ${item.name}\nDiminta: ${item.qty}\nStok Tersedia: ${item.stock}\n\nKurangi Qty atau Simpan sebagai DRAFT saja.`);
                    return;
                }
            }
            if (!picName || !signatureBase64) { alert("PIC & TTD wajib!"); return; }
        }
        ...
        <main className="min-h-screen bg-slate-50 pb-24 font-sans">
            <div className="bg-slate-900 p-6 text-white flex justify-between items-center shadow-lg sticky top-0 z-10">
                <div>
                    ...
                    <p className="text-[10px] text-slate-400 uppercase tracking-widest">Sedayu Solar Warehouse</p>
                </div>
            ...
            <div className="p-4 space-y-6">
                {fetchingDraft ? <div className="text-center py-20 font-bold animate-pulse text-slate-400">LOADING...</div> : (
                    <>
                        {!showScanner ? (
                            <button onClick={() => setShowScanner(true)} className="w-full bg-blue-600 text-white font-black py-4 rounded-2xl shadow-lg active:scale-95 transition-all text-sm">
                                📷 SCAN / TAMBAH BARANG
                            </button>
                        ) : (
                        ...
                        <div className="space-y-3">
                            {cart.map((item: any, index: number) => (
                                <div key={index} className={`bg-white p-4 rounded-2xl shadow-sm border flex justify-between items-center ${item.stock <= 0 ? 'border-red-200 bg-red-50' : 'border-slate-100'}`}>
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2">
                                            <p className="font-bold text-sm text-slate-800">{item.name}</p>
                                            {item.stock <= 0 && <span className="bg-red-500 text-white text-[8px] px-1.5 py-0.5 rounded-md font-black animate-pulse">HABIS</span>}
                                        </div>
                                        <p className={`text-[10px] font-bold ${item.stock <= 0 ? 'text-red-500' : 'text-slate-400'}`}>
                                            Stok Tersedia: {item.stock}
                                        </p>
                                        <p className="text-[9px] font-mono text-slate-300">{item.qr_id}</p>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <input
                                            ...
                                            className={`w-12 text-center border-2 rounded-lg font-black text-sm ${Number(item.qty) > Number(item.stock) ? 'border-red-500 text-red-600 bg-white' : 'border-slate-100 text-blue-600'}`}
                                        />
                                        <button onClick={() => setCart(cart.filter((_: any, i: number) => i !== index))} className="text-red-400 p-1 font-bold">✕</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        {cart.length > 0 && (
                            <div className="bg-white p-6 rounded-3xl shadow-xl border border-slate-100 space-y-4">
                                <div className="space-y-3">
                                    ...
                                    <input type="text" placeholder="Masukkan Nama Proyek" value={projectName} onChange={(e: any) => setProjectName(e.target.value)} className="w-full p-3.5 bg-slate-50 rounded-xl border-none text-slate-700 font-medium" />

                                    <label className="text-[9px] font-black text-slate-400 uppercase ml-1">Nama PIC Penerima</label>
                                    <input type="text" placeholder="Nama Teknisi" value={picName} onChange={(e: any) => setPicName(e.target.value)} className="w-full p-3.5 bg-slate-50 rounded-xl border-none text-slate-700 font-medium" />

                                    <label className="text-[9px] font-black text-slate-400 uppercase ml-1">Tanggal Keluar</label>
                                    <input type="date" value={checkoutDate} onChange={(e: any) => setCheckoutDate(e.target.value)} className="w-full p-3.5 bg-slate-50 rounded-xl border-none text-slate-700 font-medium" />
                                </div>

                                <div className="space-y-2 pt-2">
                                    <div className="flex justify-between items-center"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Tanda Tangan</label><button onClick={() => canvasRef.current?.getContext('2d')?.clearRect(0, 0, 500, 300)} className="text-[10px] text-blue-500 font-black">RESET</button></div>
