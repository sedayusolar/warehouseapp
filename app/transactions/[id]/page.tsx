...
export default function TransactionDetail({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (loading) return <div className="p-20 text-center font-bold animate-pulse text-slate-400">LOADING DETAIL...</div>;
    ...
    return (
        <main className="min-h-screen bg-slate-50 pb-24 font-sans text-slate-900">
            <div className="bg-slate-900 p-6 text-white shadow-lg sticky top-0 z-10 flex justify-between items-center">
                ...
                <button onClick={() => router.push('/transactions')} className="text-xs bg-slate-800 px-4 py-2 rounded-full font-bold">TUTUP</button>
            </div>
            ...
            <div className="p-4 max-w-2xl mx-auto space-y-8">
                ...
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p className="font-bold">{header.pic_name || '—'}</p>
                    </div>
                    <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase">Tanggal</p>
                        <p className="font-bold">{header.checkout_date}</p>
                    </div>
                    <div className="col-span-2 pt-2 border-t">
                        ...
                        ) : <p className="text-xs italic text-slate-400">Tidak ada TTD</p>}
                    </div>
                </div>
                ...
                {header.manager_approval_status === 'PENDING' ? (
                    <div className="bg-white p-6 rounded-3xl border-2 border-blue-100 shadow-xl space-y-5">
                        <h2 className="font-black text-blue-600 text-[10px] uppercase tracking-[0.2em] text-center">Manager Approval</h2>
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center">
                                ...
                                <button onClick={() => canvasRef.current?.getContext('2d')?.clearRect(0, 0, 500, 300)} className="text-[10px] text-blue-500 font-black">RESET</button>
                            </div>
                            <canvas
                    ...
                    <div className={`p-6 rounded-3xl border-2 text-center space-y-4 ${header.manager_approval_status === 'APPROVED' ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'}`}>
                        <div>
                            <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Status Manager</p>
                            <p className={`text-2xl font-black ${header.manager_approval_status === 'APPROVED' ? 'text-emerald-600' : 'text-red-600'}`}>{header.manager_approval_status}</p>
                        </div>
                        {header.manager_comment && <p className="text-xs text-slate-600 italic">"{header.manager_comment}"</p>}
                        {header.manager_signature_path && (
                            <div className="flex flex-col items-center pt-2 border-t border-white">
                                <p className="text-[9px] font-black text-slate-400 uppercase mb-2">Signature Manager</p>
