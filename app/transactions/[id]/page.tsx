...
export default function TransactionDetail({ params }: { params: Promise<{ id: string }> }) {
    ...
    const draw = (e: any) => {
        ...
        ctx.lineTo(x, y);
        ctx.stroke();
        if (e.touches) e.preventDefault(); // Cegah scroll layar saat TTD
    };
    ...
    const handleStatusUpdate = async (status: 'APPROVED' | 'REJECTED') => {
        ...
        if (status === 'APPROVED') {
            ...
            // Validasi stok fisik terakhir sebelum potong
    ...
    return (
        <main className="min-h-screen bg-slate-50 pb-24 font-sans text-slate-900">
            ...
            <div className="p-4 max-w-2xl mx-auto space-y-8">
                ...
                {/* --- LIST BARANG (DIVIDED BY CATEGORY) --- */}
                ...
                {header.manager_approval_status === 'PENDING' ? (
                    <div className="bg-white p-6 rounded-3xl border-2 border-blue-100 shadow-xl space-y-5">
                        ...
                        <textarea
                            placeholder="Berikan catatan atau instruksi khusus..."
                            value={managerComment}
                            onChange={(e) => setManagerComment(e.target.value)}
                            className="w-full p-4 bg-slate-50 rounded-2xl text-sm outline-none border-none focus:ring-2 ring-blue-500 transition-all text-slate-700"
                            rows={2}
                        />
                    ...
                    <div className={`p-8 rounded-3xl border-2 text-center space-y-4 ${header.manager_approval_status === 'APPROVED' ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'
                        ...
                        <div>
                            <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Status Final</p>
                        ...
                        {header.manager_comment && (
                            <p className="text-xs text-slate-600 italic">"{header.manager_comment}"</p>
                        )}
