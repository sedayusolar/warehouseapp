...
function TransactionListContent() {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await fetch(`https://sedayu.com/api/warehouse/delete_transaction.php?id=${id}`, { method: 'DELETE' });
            const result = await res.json();
            if (result.status === 'success') {
                alert("Terhapus!");
                fetchTransactions();
    ...
    const filteredData = transactions.filter(item => {
        ...
        if (filter === 'SUBMITTED') {
            // POINT 2: Hanya muncul yang status PENDING
    ...
    return (
        <main className="min-h-screen bg-slate-50 pb-24 font-sans">
            <div className="bg-slate-900 p-6 text-white shadow-lg sticky top-0 z-20">
                ...
                <div className="flex gap-2 mt-6 max-w-4xl mx-auto">
                    {['ALL', 'DRAFT', 'SUBMITTED'].map((tab) => (
                        <button key={tab} onClick={() => setFilter(tab)} className={`flex-1 py-2.5 text-[10px] font-black rounded-xl uppercase tracking-widest transition-all ${filter === tab ? 'bg-white text-slate-900 shadow-md' : 'bg-slate-800 text-slate-500'}`}>
            ...
            <div className="p-4 max-w-4xl mx-auto space-y-4">
                {loading ? <div className="text-center py-20 text-slate-400 font-bold text-[10px] uppercase">Sinkronisasi Data...</div> : filteredData.map((trx) => (
                    <div key={trx.id} className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100 relative">
                        ...
                        <div className="flex gap-2">
                            {trx.transaction_status === 'DRAFT' && user.role !== 'MANAGER' ? (
                                ...
                                <button onClick={() => router.push(`/transactions/${trx.id}`)} className={`flex-1 text-[10px] font-black py-4 rounded-2xl uppercase tracking-widest ${trx.manager_approval_status === 'PENDING' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-500'}`}>
